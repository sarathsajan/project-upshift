...
box_width = 26
final_log = []
market_open_ist_tz = dt.time(9, 15, 0, tzinfo=zoneinfo.ZoneInfo("Asia/Kolkata"))
market_close_ist_tz = dt.time(15, 15, 0, tzinfo=zoneinfo.ZoneInfo("Asia/Kolkata"))
...
telegram_bot.send_telegram_message(f"starting bot...")
while True:
    try:
        ...
        if datetime_now_ist_tz.time() >= market_close_ist_tz:
            print(f"market is closed as of {datetime_now_ist_tz.time()}")
            telegram_bot.send_telegram_message(f"stopping bot as market closed for today")
            break
        elif datetime_now_ist_tz.time() <= market_open_ist_tz:
            print(f"market is not yet open as of {datetime_now_ist_tz.time()}")
            print(f"sleeping for {sleep_min} minutes...")
            time.sleep(60 * sleep_min)
            continue
        
        # STEP 1
        # Get user info and the list of instruments from the holdings to be trade 
        ...
        #       IF no open positions for the day
        ...
        with open(f"logs/log_{datetime_now_ist_tz.strftime('%Y%m%d_%H0000')}.log", "w", encoding="utf-8") as log_file:
        ...
        print(f"Exception when calling Upstox API: {e}")
        telegram_bot.send_telegram_message(f"Upstox API error: {e}")
    except Exception as e:
        print(f"Unknown error caught: {e}")
        telegram_bot.send_telegram_message(f"Unknown error caught: {e}")